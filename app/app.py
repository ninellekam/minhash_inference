...
# Инициализация MinHash LSH
logger.info(f"Loading MinHash LSH from {config.MINHASH_PATH}")
lsh = LMDBMinHashLSH(
    path=config.MINHASH_PATH,
    num_perm=config.MINHASH_NUM_PERM,
    num_buckets=config.MINHASH_NUM_BUCKETS,
    num_bits=config.MINHASH_NUM_BITS,
...
def search():
    try:
        ...
        response = es.mget(index=config.ES_INDEX_NAME, body={"ids": ids})
