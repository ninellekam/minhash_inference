...
import logging
from logging.handlers import RotatingFileHandler
from utils import sliding_window_snippets
from pathlib import Path
from elasticsearch import Elasticsearch
from datetime import datetime
...
logger = logging.getLogger("elasticsearch-api")
...
def search_snips_es(
    ...
    response = [i.get('_source') for i in es.search(index=es_index_name, body=query)['hits']['hits']]

    return [
            f'#{s.get("file_abspath")}\n{s.get("snippet_text")}\n{s.get("postfix_text") if s.get("postfix_text") else ""}\n'
            for s in response[::-1]
            ]

@app.route('/info', methods=['GET'])
def info():
    return jsonify({"status": "ok", "message": "Elasticsearch API is running"}), 200
...
def search():
    try:
        ...
        # Используем новую функцию поиска через Elasticsearch
        answers = search_snips_es(
        ...
        response = {
            'results': answers,
            'status': 'success' if answers else 'empty',
            'execution_time_ms': elapsed,
            'count': len(answers)
        }
        return jsonify(response)
